...
-(void)autoRefresh{
    ...
    if(currentTab == 2 && settingsVisible == 0){
        NSLog(@"#*#*#*#*#*#* Screen 2 : Auto Refreshing UIWebView #*#*#*#*##*#*");
        [self loadURL];
    }
}
...
{
    return YES; //return UIInterfaceOrientationIsPortrait(interfaceOrientation);
}
...
- (void)loadSettings {
    ...
    [defaults setInteger:2 forKey:@"CurrentTab"];
    [defaults setInteger:0 forKey:@"SettingsVisible"];
    
    _Title = [defaults objectForKey:@"Tab2Title"];
    _URL = [defaults objectForKey:@"Tab2URL"];
    _User = [defaults objectForKey:@"Tab2User"];
    _Password = [defaults objectForKey:@"Tab2Password"];
    _RefreshInterval = [defaults integerForKey:@"Tab2RefreshInterval"];
    _LoadingStyle = [defaults integerForKey:@"Tab2LoadingStyle"];
    
    if(_Title.length == 0) {
        self.myNavigationTitle.title = @"Scene 2";
    }
    else {
